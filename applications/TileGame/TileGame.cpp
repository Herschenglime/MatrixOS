...
void TileGameAPP::KeyEventHandler(uint16_t keyID, KeyInfo* keyInfo) {
...
void TileGameAPP::MidiEventHandler(MidiPacket midiPacket) {
...
void TileGameAPP::UIMenu() {
  ...
  UIButtonWithColorFunc colorSelector(
      ...
      [&]() -> void {                               // Callback function when the button is pressed
        ...
        MatrixOS::UIInterface::ColorPicker(color);  // Refences to the color variable. The color variable will be updated by the ColorPicker function. Return true if color is
  ...
  UIButtonLarge brightnessBtn("Brightness",     // Name
                              Color(0xFFFFFF),  // Color
                              Dimension(2, 2),  // Size of the button
                              [&]() -> void {
                                MatrixOS::SYS::NextBrightness();
  ...
  MLOGI("TileGame", "Exited UI Menu");
}

void TileGameAPP::End() {
  MLOGI("TileGame", "TileGame Exited");
}
