...
void Game::spawnTile() {
  ...
  uint8_t maxIndex = grid.emptyTiles.size();
  uint8_t emptyTileIndex = rand() % maxIndex;

  Tile& tile = *grid.emptyTiles.at(emptyTileIndex);
  spawnTileAt(tile);
}
...
void Game::spawnTileAt(Tile& tile) {
  tile.rankUp();
  // grid.removeFromEmpty(tile); //make tile no longer a spawn candidate
